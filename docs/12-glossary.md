...
| **AMPRS** | Availability, Manageability, Performance, Recoverability, Security — VMware's five-pillar quality model. Superseded for practice use by MOSAIC's eight-pillar Lens. |
...
| **CLP** | Conceptual–Logical–Physical — abstraction progression used by VMware's VCF Architecture and Design Guide and inherited (extended) by MOSAIC. |
...
| **Implementation** | Specific vendor realization of a Pattern. See [Section 11](11-pattern-library.md). |
...
| **NVD** | Nutanix Validated Design. An Implementation source within the pattern library. |
...
| **RACR** | Requirements, Assumptions, Constraints, Risks — VMware's four-element input model. Superseded by MOSAIC's CADRE. |
| **Restrictions** | The **R** in CADRE. Constraints, compliance mandates, and regulatory obligations. |
| **SABSA** | Sherwood Applied Business Security Architecture. Source of the six-layer Spine. |
| **SLO / SLI / EB** | Service Level Objective, Service Level Indicator, Error Budget. Google SRE constructs adopted by MOSAIC's L5 (Operational) layer. |
...
| **VCF / VVD** | VMware Cloud Foundation / VMware Validated Design. Vendor reference designs that may serve as Implementation sources within the MOSAIC pattern library. |
| **VKS** | vSphere Kubernetes Service — Broadcom's renamed Tanzu Kubernetes Grid Service (2025). |
