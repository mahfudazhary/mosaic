...
It solves a specific problem: every major vendor (VMware/Broadcom, Nutanix, Red Hat, AWS, Azure, GCP, Cisco, Dell, NetApp) maintains its own architecture framework with its own vocabulary, and clients increasingly run hybrid estates that touch several of them. MOSAIC consumes any vendor's reference design and produces a coherent, defensible architecture artifact set.
...
- **A vendor-neutral pattern library** where each architectural pattern carries multiple implementation overlays. The practice can move a client between vendors by swapping the overlay, not the design.

## Why it matters to the practice

Three commercial pressures motivate MOSAIC.

1. **Vendor disruption.** The Broadcom acquisition of VMware destabilized incumbent VMware estates and accelerated client demand for vendor-neutral architecture advice. Clients want to know whether to renew, displace, or rationalize — and they want that advice expressed in language not anchored to any single vendor.
...
2. It enforces an explicit **pattern–implementation separation** that lets a single pattern carry multiple vendor realizations.
